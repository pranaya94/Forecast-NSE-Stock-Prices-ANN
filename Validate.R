...
nse_stock_neural_net <- function(data_vars,theta1,theta2,init_w = 0, learn = 0, alpha = 0.01)
{
  ...
  for(i in 1:nrow(data_vars))
  {
    ...
    if(learn == 1)  
    {
      ...
      delta2 = ((t(theta2) %*% delta3) * (a2 * (1 - a2)))[2:6,] #backpropagating
      
      t1_DELTA = t1_DELTA + (matrix(delta2,5,1) %*% matrix(a1,1,11)) 
      t2_DELTA = t2_DELTA + (delta3 %*% a2) #accumulating errors in this batch
    }
    else if(learn != 1 && init_w != 1) {
...
{  
  ...
  share_data_scaled = share_data_frame_numeric %>% 
    ...
    set.seed(2)
    s=base::sample(1:nrow(share_data_scaled),0.7*nrow(share_data_scaled))
    data_train=share_data_scaled[s,]
    data_test=share_data_scaled[-s,]
  
    if(lag == 3){
    
    lagged_train_data <- slide(data_train, Var = "x11", slideBy = lag) %>% 
      mutate(x11 = x113) %>% 
      select(-x113) %>% 
      filter(!is.na(x11))
    
    lagged_test_data <- slide(data_test, Var = "x11", slideBy = lag) %>% 
      mutate(x11 = x113) %>% 
      select(-x113) %>% 
      filter(!is.na(x11))
    }        
    ...
    lagged_test_data_matrix = data.matrix(lagged_test_data)
    lagged_train_data_matrix = data.matrix(lagged_train_data)
    
    #initialize weights for NN
    theta_list <- nse_stock_neural_net(lagged_train_data_matrix,theta1,theta2,1,1,0.01) 
    
    #training loop
    for (n in 1:100)
    {
      theta_list <- nse_stock_neural_net(lagged_train_data_matrix,theta_list[[1]],theta_list[[2]],0,1,0.01)
      n = n + 1
    }
    
    #validate on test data
    theta_list <- nse_stock_neural_net(lagged_test_data_matrix,theta_list[[1]],theta_list[[2]])
